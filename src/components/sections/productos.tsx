    ...
    <Section id="servicios" tone="paper">
      <SectionHead label="Servicios">
        <Headline className="text-ivory">
          Un solo sistema. <Em className="text-lime">Ocho</Em> puertas de entrada.
        </Headline>
        <p className="mt-6 max-w-xl text-pretty leading-relaxed text-ivory/60">
      ...
      <div className="reveal-stagger glass overflow-hidden rounded-card">
        {services.map((s, i) => (
          <Link
            key={s.slug}
            href={`/servicios/${s.slug}`}
            style={{ "--i": i } as CSSProperties}
            className={`group grid gap-x-8 gap-y-4 px-6 py-7 transition-colors duration-500 hover:bg-white/5 md:grid-cols-[3.5rem_15rem_1fr_11rem] md:items-baseline md:px-8 ${
              i > 0 ? "border-t border-hairline" : ""
            }`}
          >
            <div className="flex items-center gap-3 md:flex-col md:items-start md:gap-4">
              <span className="tnum font-mono text-xs text-lime/60">
            ...
            <div className="flex flex-col gap-2">
              <h3 className="wordmark text-sm uppercase text-ivory">{s.name}</h3>
              <span className="label text-ivory/40">{s.kind}</span>
            </div>

            <p className="max-w-xl text-pretty text-sm leading-relaxed text-ivory/60">
              {s.summary}
            </p>

            <div className="flex items-baseline justify-between gap-4 md:flex-col md:items-end md:gap-2">
              <span className="label whitespace-nowrap text-ivory/45 transition-colors group-hover:text-lime">
                {phaseOf(s.slug)}
              </span>
