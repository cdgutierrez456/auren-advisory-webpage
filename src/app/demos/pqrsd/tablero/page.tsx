  ...
  return (
    <>
      <header className="border-b border-line pb-10">
        <span className="label text-fg/40">Demo · PQRSD · {entidad.nombre}</span>
      ...
      <div className="mt-16 grid gap-14 lg:grid-cols-[1.3fr_1fr] lg:gap-20">
        <div className="min-w-0">
          <div className="flex items-baseline gap-5">
            <span className="label text-fg/55">Abiertos por dependencia</span>
            ...
            <Tabla head={["Dependencia", "Abiertos", "Vencidos", "Sin responsable", "Estado"]}>
              {r.porDependencia.map((d) => (
                <Fila key={d.dependencia}>
                  <Celda className="text-fg">{d.dependencia}</Celda>
                  <Celda>{d.total}</Celda>
