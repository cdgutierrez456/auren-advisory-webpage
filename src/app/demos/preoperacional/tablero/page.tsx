  ...
  return (
    <>
      <header className="border-b border-line pb-10">
        <span className="label text-fg/40">Demo · Preoperacional</span>
      ...
      <div className="mt-16 grid gap-14 lg:grid-cols-[1.3fr_1fr] lg:gap-20">
        <div className="min-w-0">
          <div className="flex items-baseline gap-5">
            <span className="label text-fg/55">Inspecciones de hoy</span>
            ...
            <Tabla head={["Hora", "Vehículo", "Conductor", "Hallazgos", "Resultado"]}>
              {deHoy.map((i) => (
                <Fila key={i.id}>
                  ...
                  <Celda className="whitespace-nowrap font-mono text-fg">{i.placa}</Celda>
          ...
          {sinInspeccion.length ? (
            <p className="mt-6 text-sm leading-relaxed text-fg/55">
        ...
        <div className="flex flex-col gap-10">
          <div>
            <div className="flex items-baseline gap-5">
              <span className="label text-fg/55">Ítems que más fallan</span>
              <span className="h-px flex-1 bg-line" />
            </div>
            <ul className="mt-6 flex flex-col gap-4">
              {r.ranking.map((x) => (
                <li key={x.itemId} className="flex items-center gap-4">
                  <span className="w-8 font-serif text-2xl text-fg">{x.veces}</span>
                  <span className="flex-1 text-sm text-fg/75">{x.texto}</span>
                  {x.critico ? <Pill tono="alerta">Crítico</Pill> : null}
                </li>
              ))}
              {r.ranking.length === 0 ? (
                <li className="text-sm text-fg/55">Sin hallazgos registrados.</li>
              ) : null}
            </ul>
          </div>
